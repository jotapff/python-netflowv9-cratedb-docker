...
entityName = os.environ['ENTITY_NAME']
orionIP = os.environ['ORION_IP']
orionPort = os.environ['ORION_PORT']
...
def checkEntityExist():
    url = "http://{}:{}/v2/entities/{}/attrs?options=keyValues".format(orionIP, orionPort, entityName)  
    payload = ""
    headers = {
    'fiware-service': '{}'.format("NetflowCollector"),
    'fiware-servicepath': '/{}'.format("NetflowCollector")
    }
    response = request("GET", url, headers=headers, data=payload)

    if response.status_code == "404":
        logger.error("Entity not exist! Creating... [{}]".format(error))
        return False
    else:
        return True
            

def createEntity(jsonData):
    url = "http://{}:{}/v2/entities?options=keyValues".format(orionIP, orionPort)  
    ...
    payload = jsonDumps(payloadJson)
    entityNamePath = "/{}".format(entityName)
    headers = {
        'Content-Type': 'application/json',
        'fiware-service': entityName,
        'fiware-servicepath': entityNamePath
        }
    try:
       request("POST", url, headers=headers, data=payload)   
    except Exception as Error:
        logger.error("Fail to create Entity! - {}".format(error))
...
def insertIntoDB(jsonData):
    url = "http://{}:{}/v2/entities/{}/attrs?options=keyValues".format(orionIP, orionPort, entityName)  

    payload = jsonDumps(jsonData)
    entityNamePath = "/{}".format(entityName)
    headers = {
        'Content-Type': 'application/json',
        'fiware-service': entityName,
